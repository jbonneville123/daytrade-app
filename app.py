...
# app.py — DayTrade App Pro (All-in-One, YF Strong + AI Live) — Streamlit OK
import os, time, math
...
st.title("📊 DayTrade App Pro — All-in-One (YF Strong + 🤖 AI Live)")
...
def rule_based_prob(df: pd.DataFrame):
    ...
    rv = nz(r, 50.0)
    votes.append(1 if 45 <= rv <= 65 else (0 if rv < 35 else 0))  # neutre = 1, extrême = 0
...
# ===================== Screener helper =====================
def build_screener_rows(symbols, interval, lookback, weights, atr_mult_target, is_crypto=False):
    rows = []
    for sym in symbols:
        df, used_int, _ = fetch(sym, interval, lookback, mode=st.session_state.get("data_mode","real_or_fallback"))
        if df.empty:
            rows.append({"symbol": sym, "score": 0.0, "used_interval": used_int, "price": np.nan})
            continue
        c = s_close(df); v = s_volume(df)
        a = atr(df,14); e20 = ema(c,20); e50=ema(c,50); e200=ema(c,200)
        vw = vwap(df)
        price = nz(c, 0.0)
        mom = float((price / float(c.iloc[max(len(c)-21,0):].min())) - 1.0) if len(c)>=21 else 0.0
        volat = float(nz(a,0.0) / price) if price>0 else 0.0
        liq = float(np.log10(max(nz(v,1.0),1.0)))
        tq = float(((price - nz(e50,np.nan)) / max(price,1e-9)) + (1 if nz(e20,np.nan) > nz(e50,np.nan) > nz(e200,np.nan) else 0))
        score = (weights.get("momentum",0.4)*mom +
                 weights.get("volatility",0.2)*volat +
                 weights.get("liquidity",0.2)*liq +
                 weights.get("trend_quality",0.2)*tq)
        rows.append({"symbol": sym, "score": round(score,4), "used_interval": used_int, "price": price})
    return rows
        ...
        if df.empty:
            ...
            def ai_predict(df, capital=100000.0, risk_pct=0.005, buy_th=0.6, sell_th=0.4):
                ...
                if len(feats) < 60:
                    ...
                    a = atr(df,14); entry = nz(c,0.0); stop = entry - 2.0 * nz(a,0.0) if p>=0.5 else entry + 2.0 * nz(a,0.0)
                    ...
                    return p, action, entry, stop, size, "rule-based"
                ...
                x_last = Xz[-1:]
                w, b = train_logreg_gd(X_train, y_train, lr=0.2, epochs=250, l2=1e-4)
                p_up = float(predict_proba(x_last, w, b)[0])
                a = atr(df,14); entry = float(c.loc[data.index[-1]]); atr_last = nz(a,0.0)
                stop = entry - 2.0 * atr_last if p_up >= 0.5 else entry + 2.0 * atr_last
                action = "BUY" if p_up >= 0.6 else ("SELL" if p_up <= 0.4 else "HOLD")
                size = int((capital * 0.005) // max(abs(entry - stop), 1e-9)) if action != "HOLD" else 0
            ...
            colw.metric("Stop", f"{stop:.2f}")
            st.caption(f"Mode AI: **{mode_ai}** — taille conseillée: **{size}** unités (risk=0.5%).")

        if auto:
            st.info(f"Auto-refresh dans {every}s… (désactive la case pour stopper)")
