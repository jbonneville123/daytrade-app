...
def safe_avg_dollar_volume(df: pd.DataFrame, window: int = 30) -> float:
    # Robust to missing/NaN without ambiguous truth checks
    if df is None or df.empty or "Close" not in df.columns or "Volume" not in df.columns:
        return 0.0
    c = pd.to_numeric(df["Close"].tail(window), errors="coerce")
    v = pd.to_numeric(df["Volume"].tail(window), errors="coerce")
    prod = (c * v).dropna()
    ...
    if mode.startswith("Equities"):
        ...
        if run and symbols:
            ...
            for sym in symbols:
                ...
                if df.empty:
                    continue
                price = float(pd.to_numeric(df["Close"].tail(1), errors="coerce").iloc[-1])
                # new robust volume checks
                avg_vol = float(pd.to_numeric(df["Volume"].tail(30), errors="coerce").mean() or 0.0)
                if price < min_price: 
                    continue
