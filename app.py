...
def safe_close(df: pd.DataFrame) -> pd.Series:
    """Always return a 1-D float Series for Close, even if yfinance returns multi-cols."""
...
def generate_signals(df: pd.DataFrame, ema_period=20, atr_period=14, atr_mult=2.0, or_minutes=15, allow_short=True):
    if df.empty:
        return df
    df = df.copy()
    df["Close"] = safe_close(df)
    df["Volume"] = safe_volume(df)
...
def safe_avg_dollar_volume(df: pd.DataFrame, window: int = 30) -> float:
    if df is None or df.empty:
        return 0.0
    c = safe_close(df).tail(window)
    v = safe_volume(df).tail(window)
    prod = (c * v).dropna()
    ...
    if mode.startswith("Equities"):
        ...
        if run and symbols:
            ...
            for sym in symbols:
                ...
                if df.empty:
                    continue
                close_series = safe_close(df)
                price = float(close_series.tail(1).iloc[-1]) if not close_series.tail(1).isna().all() else 0.0
                avg_vol = float(safe_volume(df).tail(30).mean() or 0.0)
                if price < min_price: 
                    continue
                ...
                potential = float(atr_mult_target * atr_s.iloc[-1]) if potential_method=="atr_target" else float(max(0.0, close.iloc[-60:].max()-close.iloc[-1]))
